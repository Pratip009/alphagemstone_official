...
function normalizeRow(
  ...
  if (Object.values(raw).every((v) => String(v ?? "").trim() === "")) {
    return { data: {} };
  }
  try {
    ...
    // ── price ─────────────────────────────────────────────────────────────────
    const parsedPrice = parseFloat(get("price"));
const priceRaw =
  isNaN(parsedPrice) || parsedPrice < 0
    ...
    if (shapeRaw.length > 0) {
      shape = shapeRaw.map((s) => (SHAPES.includes(s as never) ? s : "other"));
    }
    ...
    if (colorRaw.length > 0) {
      color = colorRaw.map((c) => (COLORS.includes(c as never) ? c : "other"));
    }

    // ── clarity ───────────────────────────────────────────────────────────────
    let clarity: string[] | undefined;

    const clarityRaw = parseMulti(get("clarity").toUpperCase());

    if (clarityRaw.length > 0) {
      clarity = clarityRaw.map((c) =>
        CLARITIES.includes(c as never) ? c : "other",
      );
    }
    ...
    if (certRaw && certRaw.toLowerCase() !== "none") {
      ...
      certification =
        certs.length > 0
          ? certs.map((c) => (CERTIFICATIONS.includes(c as never) ? c : "none"))
          : ["none"];
    }
    ...
    const images = imagesRaw
      ...
      : [];
    const placeholder =
      "https://res.cloudinary.com/dgsmpvy9i/image/upload/v1783011226/61IYf8jB4tL._AC_UF894_1000_QL80__c1lln5.jpg";
    ...
    if (!categoryRaw)
      return { data: {}, error: "Missing required field: category" };
...
export async function parseExcel(buffer: Buffer): Promise<ParseResult> {
  ...
  if (!sheetName) {
    return {
      rows: [],
      parseErrors: [{ row: 0, error: "No sheets found in Excel file" }],
    };
  }
