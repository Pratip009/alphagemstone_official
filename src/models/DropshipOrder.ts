...
export interface IDropshipOrder extends Document {
  ...
  // One-time id generated by the seller's browser per submit attempt. A
  // double-click / network retry re-sends the same id and gets the SAME
  // order back instead of creating (and reserving stock for) a duplicate.
  clientRequestId?: string;
  // Short-lived lock so two simultaneous capture calls for the same order
  // can never both talk to PayPal / both flip the order's state.
  captureLockUntil?: Date;
  paidAt?: Date;
  cancelledAt?: Date;
  cancelledBy?: "seller" | "admin" | "system";
  // Raised when something needs a human (e.g. payment captured but the
  // stock had already been released). Surfaced in the admin Orders tab.
...
const DropshipOrderSchema = new Schema<IDropshipOrder>(
  {
    ...
    captureLockUntil: { type: Date },
    paidAt: { type: Date },
    cancelledAt: { type: Date },
    cancelledBy: { type: String, enum: ["seller", "admin", "system"] },
    needsAttention: { type: Boolean, default: false },
    attentionReason: { type: String, trim: true },
  },
  { timestamps: true }
);
...
DropshipOrderSchema.index({ labelId: 1 }, { sparse: true });
// Idempotent submit — one order per (seller, clientRequestId).
DropshipOrderSchema.index(
  { application: 1, clientRequestId: 1 },
  { unique: true, partialFilterExpression: { clientRequestId: { $type: "string" } } }
);
// Stale-order sweeper (see releaseStaleDropshipOrders).
DropshipOrderSchema.index({ status: 1, paymentStatus: 1, stockReserved: 1, createdAt: 1 });
